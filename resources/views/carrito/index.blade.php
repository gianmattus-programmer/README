@extends('layouts.app')

@section('template_title')
    Carrito de compras
@endsection

@section('estilos')
    <style>
        .mis_cursos {
            
        }
        .product_carrito {
            width: 37%;
        }
        .table th, .table td {
            padding: 5px;
            vertical-align: middle;
        }
        .cont_pt {
            padding: 50px 30px;
            border: 2px solid #d5d8dc;
        }
        .pag_ava {
            margin-top: 20px;
        }
        .pre_cont {
            color: #000;
            font-size: 14px;
            margin-bottom: 5px;
            display: flex;
            width: 100%;
            font-weight: 500;
        }
        .pre_cont span {
            width: 50%;
        }
        .form_cont {
            padding: 50px 30px;
            border: 2px solid #d5d8dc;
        }
    </style>
@endsection

@section('content')
    @include('partials.blackheader')

    <div class="section_cursos">
        <div class="container">
            <div class="row">
                <div class="col-lg-12 col-md-12">
                    <div class="title_cat">
                        <h1>Carrito de compras</h1>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="mis_cursos margeleft">
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-8 col-xs-8">
                    <div class="row rowleft">
                        @if($carts->isEmpty())
                            <p>No hay productos en el carrito</p>
                        @else
                            <table class="table align-middle mb-0">
                                <thead class="table-light">
                                    <tr>
                                        <th width="10%">Imagen</th>
                                        <th width="30%">Nombre</th>
                                        <th width="10%">Categoria</th>
                                        <th width="10%">Precio</th>
                                        <th width="10%">Descuento</th>
                                        <th width="10%">Cantidad</th>
                                        <th width="10%">Total</th>
                                        <th></th>
                                    </tr>
                                </thead>
                            
                                <tbody>
                                    @foreach($carts as $cart)
                                        <tr>
                                            <td>
                                                <img src="{{ asset('panel/cursos/listados') }}/{{ $cart->product->file }}" class="product_carrito" alt="product img">
                                            </td>

                                            <td>{{ $cart->product->nombre }}</td>
                                            <td>{{ $cart->categoria }}</td>
                                            <td>{{ 's/ '.number_format($cart->precio, 2, '.', '') }}</td>
                                            <td>{{ 's/ '.number_format($cart->descuento, 2, '.', '') }}</td>

                                            <td>{{ $cart->quantity }}
                                                <!-- form action="{{ route('cart.update', $cart->id) }}" method="POST">
                                                    @csrf
                                                    <input type="number" name="quantity" value="{{ $cart->quantity }}" min="1">
                                                    
                                                    <button type="submit">Actualizar</button>
                                                </form -->
                                            </td>

                                            <td>
                                                @if($cart->descuento)
                                                    {{ 's/ '.number_format($cart->descuento * $cart->quantity, 2, '.', '') }}
                                                @else
                                                    {{ 's/ '.number_format($cart->precio * $cart->quantity, 2, '.', '') }}
                                                @endif
                                            </td>

                                            <td>
                                                <form action="{{ route('cart.remove', $cart->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')

                                                    <button type="submit">Eliminar</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        @endif
                    </div>
                </div>

                <div class="col-lg-4 col-md-4 col-xs-4">
                    <div class="prices_cart">
                        <div class="cont_pt">
                            <p class="pre_cont">
                                <span>SUBTOTAL</span> 
                                <span>S/ <?php echo number_format($total, 2, '.', ''); ?></span>
                            </p>

                            <?php
                                $igv = $total * (18 / 100);
                                $totp = $total + $igv;
                            ?>

                            <p class="pre_cont">
                                <span>IGV 18% </span>
                                <span>S/ <?php echo number_format($igv, 2, '.', ''); ?></span>
                            </p>
                            
                            <p class="pre_cont">
                                <span>TOTAL </span>
                                <span>S/ <?php echo number_format($totp, 2, '.', ''); ?></span>
                            </p>

                            <div id="pag_ava" class="pag_ava">
                                <a href="{{ url('/finalizarcompra') }}" class="btn btn-dark col-md-12">
                                    PROCEDER A PAGAR
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    @include('partials.footer')
@endsection

@section('footer_scripts')

@endsection
