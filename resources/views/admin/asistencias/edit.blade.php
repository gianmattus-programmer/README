<div class="modal fade" id="ModalEdit{{ $asistencia->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-black p-4">
                <h5 class="modal-title text-white">EDITAR ASISTENCIA</h5>
                ...
                <div class="row g-3 pt-3 pb-2 col-md-12">
                    <div class="form-group mb-2 mt-2 col-md-12">
                        <div class="row">
                                    ...
                                    <select class="form-control form-control-solid" id="profesor_id" name="profesor_id" type="select" required>
                                        @foreach($users as $user)
                                            @if($user->id == $asistencia->profesor_id)
                                                <option value="{{ $asistencia->profesor_id }}">{{ $user->first_name . ' ' . $user->last_name }}
                                            @endif
                                        @endforeach
                            ...
                            @else
                                <input type="hidden" id="user_id" name="user_id" value="{{ asistencia->user_id }}" />
                            @endrole
                                ...
                                <select class="form-control form-control-solid" id="user_id" name="user_id" type="select" required>
                                    @foreach($users as $user)
                                        @if($user->id == $asistencia->user_id)
                                            <option value="{{ $asistencia->user_id }}">{{ $user->first_name . ' ' . $user->last_name }}
                                        @endif
                                    @endforeach
                                    <option value="">Seleccionar</option>
                            ...
                            <div class="form-group mb-2 mt-2 col-md-6">
                                ...
                                <select class="form-control form-control-solid" id="listado_id" name="listado_id" type="select" required>
                                    <option value="{{ $asistencia->listado->id }}">{{ $asistencia->listado->nombre }}
                                    <option value="">Seleccionar</option>
                            ...
                            <div class="form-group mb-2 mt-2 col-md-6">
                                ...
                                <input class="form-control form-control-solid" id="fecha" name="fecha" type="date" required value="{{ $asistencia->fecha }}" />
                            </div>

                            <div class="form-group mb-2 mt-2 col-md-6">
                                <label class="mb-2">Hora</label>
                                <input class="form-control form-control-solid" id="hora" name="hora" type="time" required value="{{ $asistencia->hora }}" />
                            </div>
                        </div>
                    </div>

                    <input class="form-control form-control-solid" id="estatus" name="estatus" type="hidden" value="{{ $asistencia->estatus }}" />
                </div>
            </div>
