...
@section('content')
    ...
    @if ($message = Session::get('success'))
        <div class="alert alert-success border-0 bg-success alert-dismissible fade show py-2">
            <div class="d-flex align-items-center">
                ...
                <div class="ms-3">
                    <h6 class="mb-0 text-white font-14">Términos y condiciones - {{ $message }}</h6>
                </div>
            </div>
    ...
    <div class="card">
        <div class="card-header bg-black pt-3 pb-3">
            <div class="d-flex align-items-center">
                <div>
                    <h6 class="mb-0 title_page text-white">PÁGINA TÉRMINOS Y CONDICIONES</h6>
                </div>

                <!-- div class="d-flex ms-auto">
                    <form id="identifier" action="{{ route('adminterminos.store') }}" class="ms-1" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="card-body">
            @foreach($terminos as $termino)
                <form action="{{ route('adminterminos.update', $termino->id) }}" class="ms-1" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
            
                    <div class="row g-3 pt-3 pb-2 col-md-12">
                        <div class="form-group mb-2 mt-2 col-md-12">
                            <textarea class="informacion" id="informacion" name="informacion" cols="30" rows="10" required>{{ $termino->informacion }}</textarea>
                        </div>

                        <input id="estatus" name="estatus" type="hidden" value="{{ $termino->estatus }}" />
                    </div>
