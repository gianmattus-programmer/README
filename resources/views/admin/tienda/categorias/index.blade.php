...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-black pt-3 pb-3">
            <div class="d-flex align-items-center">
                <div>
                    <h6 class="mb-0 title_page text-white">CATEGORÍAS DE LOS PRODUCTOS</h6>
                </div>

                <div class="ms-auto">
                    <form action="{{ route('tiendacategorias.store') }}" class="ms-1" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        @include('admin.tienda.categorias.create')
                    </form>
                </div>
            </div>
        </div>
                        ...
                        @foreach($categorias as $categoria)
                            @if($categoria->estatus == "1")
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{ asset('panel/tienda/categorias') }}/{{ $categoria->file }}" class="img-responsive" width="40px" alt="">
                                    </td>

                                    <td>{{ $categoria->nombre }}</td>
                                    <td>{{ $categoria->productos_count.' productos' }}</td>
                                    <td>{{ $newDate }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex order-actions">
                                            ...
                                            @include('admin.tienda.categorias.show')

                                            <form action="{{ route('tiendacategorias.update', $categoria->id) }}" class="ms-1" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                ...
                                                @include('admin.tienda.categorias.edit')
                                            </form>
                                            ...
                                            <form action="{{ route('estatus.update', $categoria->id) }}" class="ms-1" method="POST">
                                                ...
                                                <input type="hidden" value="tiendacategorias" id="pagina" name="pagina" />

                                                @include('admin.tienda.categorias.predelete')
                                            </form>
                                        </div>
                                    </td>
                                </tr>
