...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-black pt-3 pb-3">
            <div class="d-flex align-items-center">
                <div>
                    <h6 class="mb-0 title_page text-white">CATEGORÍAS DE LOS CURSOS</h6>
                </div>

                <div class="ms-auto">
                    <form action="{{ route('cursoscategorias.store') }}" class="ms-1" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        @include('admin.cursos.categorias.create')
                    </form>
                </div>
            </div>
        </div>
                        ...
                        @foreach($categorias as $categoria)
                            @if($categoria->estatus == "1")
                                ...
                                <tr>
                                    ...
                                    <td>{{ $categoria->cursos_count.' cursos' }}</td>
                                    <td>{{ $newDate }}</td>
