...
class ProfilesController extends Controller
{
    ...
    public function show($username)
    {
        ...
        // Obtén los cursos comprados directamente con sus detalles y listados
        $checkouts = Checkout::where('user_id', $user->id)->with('detalles.listado.temarios')->get();
        
        $restantes = CursoListado::all()->take(4);
        ...
        $cursosCompletos = 0;
        $temariosCompletos = 0;
        $totalCursos = $checkouts->count();

        foreach ($checkouts as $checkout) {
            $detalles = Detalle::where('checkout_id', $checkout->id)->get();
    
            foreach ($detalles as $detalle) {
                $listado = CursoListado::find($detalle->listado_id);
                if ($listado) {
                    $temarios = $listado->temarios;
                    $temariosCompletados = $temarios->where('examen', 2)->count();
    
                    if ($temariosCompletados === $temarios->count()) {
                        $cursosCompletos++;
                    }
    
                    $temariosCompletos += $temariosCompletados;
                }
            }
        }

        $porcentajeCursosCompletos = 0;

        if ($totalCursos > 0) {
            $porcentajeCursosCompletos = ($temariosCompletos / ($totalCursos * 2)) * 100;
        }
        ...
        $data = [
            ...
            'tcategorias'       => $tcategorias,
            'tlistados'         => $tlistados,
            'porcentaje'        => round($porcentajeCursosCompletos, 2),
        ];
